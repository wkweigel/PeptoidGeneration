...
def cyclic_peptoid_smiles_from_string(peptoid_string:str, peptoid_dict:dict):
    '''Convert peptoid string into its corresponding smiles.
    Parameters:
        peptoid_string: An string representing the monomers in the desired peptoid. 
        peptoid_dict: A dictionary of corresponding monomer codes and smiles.
    ...
    #Start with protected bromoacetic acid
    peptoid=Reaction(int_halide=peptoid_dict['Z'])

    for idx, monomer in enumerate(peptoid_string): 
        if monomer == 'X':
            if idx==0:
                peptoid.int_halide_smiles=peptoid.amine_displacement(peptoid_dict[monomer],peptoid.int_halide_smiles)
            else:
    ...
    peptoid.int_amine_smiles=peptoid.amine_displacement(peptoid_dict['Q'],peptoid.int_halide_smiles)
    peptoid.int_amine_smiles=peptoid.intramolecular_click(peptoid.int_amine_smiles)
